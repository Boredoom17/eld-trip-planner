...
const makeMarker = (emoji, bg, size = 36) => L.divIcon({
  className: '',
  html: `<div style="width:${size}px;height:${size}px;background:${bg};border-radius:50% 50% 50% 4px;transform:rotate(-45deg);border:3px solid white;box-shadow:0 3px 14px rgba(0,0,0,0.45);display:flex;align-items:center;justify-content:center;"><span style="transform:rotate(45deg);font-size:${size*0.42}px;line-height:1;">${emoji}</span></div>`,
...
function FitBounds({ allPoints }) {
  ...
  useEffect(() => {
    if (allPoints.length > 1 && !didFit.current) {
...
export default function TripMap({ coordinates, routeGeometry, stops, isVisible = true }) {
  ...
  const center = [(coordinates.current.lat + coordinates.dropoff.lat) / 2, (coordinates.current.lng + coordinates.dropoff.lng) / 2]
  const mappableStops = stops.filter(s => ['start', 'pickup', 'dropoff', 'fuel'].includes(s.type))
  ...
  return (
    <div style={{ position: 'relative', height: '100%', width: '100%' }}>
            ...
            <Marker key={i} position={pos} icon={icon}>
              <Popup>
                <div style={{ fontFamily: 'DM Sans, sans-serif', lineHeight: 1.6, minWidth: '200px' }}>
                  <div style={{ fontWeight: 700, fontSize: '14px', marginBottom: '4px' }}>{stop.label}</div>
                  {stop.location && stop.location !== 'En route' && <div style={{ fontSize: '11px', color: '#888', marginBottom: '4px' }}>📍 {stop.location}</div>}
                  <div style={{ fontSize: '11px', color: '#666', marginBottom: '4px' }}>🕐 {formatPopupTime(stop.arrival_time)}</div>
      ...
      <div style={{ position: 'absolute', bottom: '20px', left: '12px', zIndex: 1000, background: 'rgba(15,30,50,0.93)', border: '1px solid #2a4060', borderRadius: '12px', padding: '12px 14px', backdropFilter: 'blur(12px)', boxShadow: '0 4px 24px rgba(0,0,0,0.5)', minWidth: '195px' }}>
        <div style={{ fontSize: '9px', fontWeight: 700, letterSpacing: '0.12em', color: '#8ba0b8', marginBottom: '10px', fontFamily: 'Syne, sans-serif' }}>MAP LEGEND</div>
        {LEGEND.map((item, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '7px' }}>
            {item.line ? <div style={{ width: 22, height: 4, background: item.color, borderRadius: 2, flexShrink: 0 }} /> : <span style={{ fontSize: '14px', flexShrink: 0 }}>{item.emoji}</span>}

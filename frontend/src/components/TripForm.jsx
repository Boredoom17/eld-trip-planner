...
function LocationInput({ id, label, value, onChange, placeholder, icon }) {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.detail.activeId !== id) { setOpen(false); setActiveIdx(-1) }
    }
    ...
    <div ref={wrapRef} style={{ position: 'relative', marginBottom: '6px' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          style={{
            ...
            background: 'var(--navy)',
            border: `1.5px solid ${focused ? 'var(--accent)' : 'var(--navy-border)'}`,
        ...
        {query.length > 0 && (
          <button
            ...
            style={{
              ...
              cursor: 'pointer', fontSize: '18px', lineHeight: 1, padding: '2px 4px', opacity: 0.6,
            }}
          >×</button>
        )}
      </div>
      ...
      {open && suggestions.length > 0 && (
        <div style={{
          ...
          background: '#1e2d42', border: '1.5px solid var(--navy-border)',
          ...
          {suggestions.map((s, i) => (
            <div
              ...
              style={{
                ...
                color: i === activeIdx ? '#151f2e' : 'var(--text)',
                background: i === activeIdx ? 'var(--accent)' : 'transparent',
                cursor: 'pointer',
                borderBottom: i < suggestions.length - 1 ? '1px solid var(--navy-border)' : 'none',
...
function buildDayLabels(days) {
  return days.map(day => {
    ...
    const label = `Day ${day.day_number} — ${d.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}`
    return { date: day.date, label }
  })
}

export default function TripForm({ onSubmit, loading, tripDays, savedSchedule, onScheduleSave }) {
  ...
  const dayLabels = tripDays
    ? buildDayLabels(tripDays)
    : [{ date: new Date().toISOString().slice(0, 10), label: 'Day 1 — Today' }]
  ...
  return (
    <>
      <form onSubmit={handleSubmit}>
        ...
        <LocationInput
          id="loc-current" label="CURRENT LOCATION" icon="🚛"
          value={form.current_location}
          onChange={v => setForm(f => ({ ...f, current_location: v }))}
          placeholder="e.g. Chicago, IL"
        />
        <div style={{ marginLeft: '15px', height: '12px', borderLeft: '2px dashed var(--navy-border)' }} />
        <LocationInput
          id="loc-pickup" label="PICKUP LOCATION" icon="📦"
        ...
        <div style={{ marginLeft: '15px', height: '12px', borderLeft: '2px dashed var(--navy-border)' }} />
        <LocationInput
          id="loc-dropoff" label="DROPOFF LOCATION" icon="✅"
        ...
        <div style={{ marginTop: '18px', marginBottom: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '7px' }}>
            ...
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '15px', color: sliderColor }}>
              {form.cycle_used_hours}
              <span style={{ color: 'var(--text-muted)', fontWeight: 400, fontSize: '11px' }}> / 70 hrs</span>
            </span>
          </div>
          <input
            type="range" min={0} max={69.5} step={0.5}
            value={form.cycle_used_hours}
            onChange={e => setForm(f => ({ ...f, cycle_used_hours: parseFloat(e.target.value) }))}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '3px' }}>
            ...
            <span style={{ fontSize: '10px', color: pct > 85 ? '#ff5252' : 'var(--text-muted)' }}>
              {pct > 85 ? '⚠ Near limit!' : '70 hr max'}
            </span>
          </div>
          <div style={{ marginTop: '6px', height: '4px', borderRadius: '2px', background: 'var(--navy)', overflow: 'hidden' }}>
            <div style={{
              height: '100%', width: `${pct}%`,
              background: sliderColor, borderRadius: '2px',
              transition: 'width 0.3s, background 0.3s',
            }} />
          </div>
          <div style={{
            marginTop: '8px', padding: '8px 10px',
            background: 'var(--navy)', borderRadius: '7px',
        ...
        <button
          ...
          style={{
            ...
            background: savedSchedule ? 'rgba(105,240,174,0.1)' : 'var(--navy-card)',
            border: `1.5px solid ${savedSchedule ? '#69f0ae' : 'var(--navy-border)'}`,
            borderRadius: '8px',
            color: savedSchedule ? '#69f0ae' : 'var(--text-muted)',
          ...
          onMouseEnter={e => { if (!savedSchedule) e.currentTarget.style.borderColor = 'var(--accent)' }}
          onMouseLeave={e => { if (!savedSchedule) e.currentTarget.style.borderColor = 'var(--navy-border)' }}
        >
          <span>{savedSchedule ? '✓ DRIVER SCHEDULE SAVED' : '📋 SET DRIVER SCHEDULE'}</span>
          <span style={{ fontSize: '10px', opacity: 0.7 }}>
            {savedSchedule ? 'click to edit' : 'for log sheets →'}
          </span>
        </button>
        ...
        <button
          ...
          style={{
            ...
            color: loading ? 'var(--text-muted)' : '#151f2e',
